...
class SeverancepayController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.industrial_relations.severance-pay.index', compact('datas'));
    }

    public function create(Request $request)
    {
        $pasal = Pasal::all();
        return view('admin.industrial_relations.severance-pay.create', compact('pasal'));
    }
    ...
    {
        return view('admin.industrial_relations.severance-pay.import');
    }
    ...
    public function print($id)
    {
        ...
        if ($data->pasal == '52 ayat (2)') {
            $pdf = PDF::loadView('admin.industrial_relations.severance-pay.severance-print-522', compact('data'))->setPaper('a4');
        }
        if ($data->pasal == '52 ayat (1)') {
            $pdf = PDF::loadView('admin.industrial_relations.severance-pay.severance-print-521', compact('data'))->setPaper('a4');
        }
        if ($data->pasal == '51') {
            $pdf = PDF::loadView('admin.industrial_relations.severance-pay.severance-print-51', compact('data'))->setPaper('a4');
        }
        if ($data->pasal == '16') {
            $pdf = PDF::loadView('admin.industrial_relations.severance-pay.severance-print-16', compact('data'))->setPaper('a4');
        }
        return $pdf->stream();
    }
}
